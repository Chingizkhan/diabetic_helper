...
type SugarLevelStorage struct {
	Db *sqlx.DB
	*sqlx.Tx
}

func NewSugarLevelStorage() *SugarLevelStorage {
	return &SugarLevelStorage{
		Db: db,
		Tx: nil,
	}
}

func (s *SugarLevelStorage) Begin(ctx context.Context, options *sql.TxOptions) (err error) {
...
func (s *SugarLevelStorage) Commit() error {
	if s.Tx == nil {
...
func (s *SugarLevelStorage) Add(ctx context.Context, level *storage.SugarLevel) error {
	q := `
...
func (s *SugarLevelStorage) Exists(ctx context.Context, userId string) (has bool, err error) {
...
func (s *SugarLevelStorage) Update(ctx context.Context, level storage.SugarLevel) error {
	return nil
}

func (s *SugarLevelStorage) Find(ctx context.Context, dto storage.SugarLevelFindDTO) ([]storage.SugarLevel, error) {
...
func (s *SugarLevelStorage) GetCountAllForFind(ctx context.Context, dto storage.SugarLevelFindDTO) (uint32, error) {
	var count uint32
...
func (s *SugarLevelStorage) RollbackIfError(err *error) {
	if (*err) != nil {
		_ = s.Tx.Rollback()
	}
}
